...
export default function CreateEventModal({ open, onOpenChange }) {
  ...
  // prevent setState after unmount on fast nav
  ...
  const addSpeaker = () => {
    setFormData((prev) => ({
      ...
      speakers: [
        ...prev.speakers,
        { id: Date.now(), name: "", title: "", company: "", bio: "", avatarUrl: "" },
      ],
    }));
  };
  ...
  const removeSession = (id) => {
    setFormData((prev) => ({
      ...prev,
      agenda: prev.agenda.filter((s) => s.id !== id),
    }));
  };
  const updateSession = (id, field, value) => {
    setFormData((prev) => ({
      ...prev,
      agenda: prev.agenda.map((s) => (s.id === id ? { ...s, [field]: value } : s)),
    }));
  };
  ...
  async function handleSubmit(e) {
    ...
    // block while slug check is unresolved or bad
    ...
    try {
      ...
      // 5) (optional) refresh list ahead of landing
      await fetchMyEvents?.();

      // 6) Go to My Events and pass createdId for highlight
      if (created?._id) {
        navigate("/dashboard/organizer/events", { state: { createdId: created._id } });
      } else {
