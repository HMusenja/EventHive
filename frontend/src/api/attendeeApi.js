...
export async function createAttendee(payload) {
  const { data } = await axios.post("/attendees", payload);
  return data; // { attendee }
}

export async function updateAttendee(id, payload) {
  const { data } = await axios.patch(`/attendees/${id}`, payload);
  return data; // { attendee }
}
...
export async function getEventAttendeeCountById(eventId) {
  const { data } = await axios.get(`/events/${eventId}/attendees/count`);
...
export async function getEventAttendeeCountBySlug(slug) {
  const { data } = await axios.get(`/events/${slug}/attendees/count`, {
    params: { by: "slug" },
  });
  return data;
}
